...
async function appendStoredEvents(incoming: StatisticsEvent[]): Promise<boolean> {
  if (incoming.length === 0) return false
  const current = await getStatsEvents()
  const next = appendStatisticsEvents(current, incoming)
  if (next.length === current.length) return false
  await setStatsEvents(next)
  return true
}
...
async function ensureChapterSeedForManga(manga: Manga | TrashedManga): Promise<boolean> {
  ...
  await setStatsEvents(appendStatisticsEvents(events, [seedEvent]))
  return true
}

async function ensureSnapshotSeeds(mangaList: Manga[], mangaTrash: TrashedManga[]): Promise<boolean> {
  const events = await getStatsEvents()
  ...
  await setStatsEvents(appendStatisticsEvents(events, additions))
  return true
}

export async function recordStatisticsForCreate(manga: Manga, at: number): Promise<void> {
  await appendStoredEvents(buildCreateStatisticsEvents(manga, at))
}

export async function recordStatisticsForRestore(manga: Manga, at: number): Promise<void> {
  await appendStoredEvents(buildRestoreStatisticsEvents(manga, at))
}

export async function recordStatisticsForDelete(manga: Manga, at: number): Promise<void> {
  await ensureChapterSeedForManga(manga)
  await appendStoredEvents(buildDeleteStatisticsEvents(manga, at))
}

export async function recordStatisticsForUpdate(before: Manga, after: Manga, at: number): Promise<void> {
  const additions: StatisticsEvent[] = []
  const events = await getStatsEvents()

  if (!hasChapterHistory(events, before.id)) {
  ...
  await appendStoredEvents(additions)
}

export async function recordStatisticsForImportedManga(manga: Manga, at: number): Promise<void> {
  await appendStoredEvents(buildCreateStatisticsEvents(manga, at))
}
...
async function fetchComickTags(comickHid: string): Promise<string[]> {
  const response = await fetch(`https://api.comick.dev/comic/${comickHid}?tachiyomi=true`, {
...
export async function refreshLibraryMetadata(): Promise<MetadataRefreshResult> {
  ...
  await setMangaList(nextList)
  await setStatsTagCache(cache)
  notifyStatsUpdated()
...
export async function statsGetOverview(): Promise<{ success: boolean; data?: StatisticsOverview; error?: string }> {
  try {
    const mangaList = await getMangaList()
    const mangaTrash = await getMangaTrash()
    const seedsAdded = await ensureSnapshotSeeds(mangaList, mangaTrash)
    const events = await getStatsEvents()
    const tagCache = await getStatsTagCache()
...
export async function statsRefreshTags(): Promise<{ success: boolean; data?: StatisticsOverview; error?: string }> {
  try {
    await refreshStatisticsTags()
    return statsGetOverview()
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) }
  }
}

export async function statsRefreshGenres(): Promise<{ success: boolean; data?: StatisticsOverview; error?: string }> {
  return statsRefreshTags()
}